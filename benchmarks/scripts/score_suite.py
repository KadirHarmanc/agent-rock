...
def score_case(case_manifest: dict[str, Any], report: dict[str, Any], mode: str) -> CaseResult:
    ...
    if expected_count == 0:
        clean_success = extra_count == 0
        precision = 1.0 if clean_success else 0.0
        recall = 1.0 if clean_success else 0.0
        severity_accuracy = 1.0 if clean_success else 0.0
        cwe_accuracy = 1.0 if clean_success else 0.0
    else:
